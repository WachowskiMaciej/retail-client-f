...
def test_status_maps_to_error(
    ...
    executor, _ = make_executor([httpx.Response(status)], retry=RetryPolicy(max_attempts=1))
...
def test_422_carries_field_level_details(make_executor: ExecutorFactory) -> None:
    ...
    executor, _ = make_executor([httpx.Response(422, json=body)])

    with pytest.raises(ValidationError) as excinfo:
        executor.request("POST", "/users", authenticated=True)
